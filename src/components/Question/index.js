...
const Question = (questionState, state, send) => {
  questionState.publicGuessesTotal = questionState.publicGuesses.reduce(
    (memo, value) => {
      return memo + value;
    },
    0
  ...
  questionState.publicGuessPercentages = questionState.publicGuesses.map(
    value => {
  ...
  return yo`<div class="Question${
    state.isLocked ? " Question--locked" : ""
  } Card">
  	...
  	${
      state.isLocked &&
      questionState.guess !== null &&
      questionState.answer !== null
        ? QuestionPersonalResult(questionState)
        : null
    }
  	${
      state.isLocked || questionState.guess !== null
        ? QuestionPublicTitle(questionState, state)
        : null
    }
  	${
      (state.isLocked && questionState.answer !== null) ||
      questionState.guess !== null
        ? QuestionPublicResult(questionState, state)
        : null
    }
  	${
      state.isLocked || questionState.guess !== null
        ? QuestionPublicChoices(questionState, state)
        : null
    }
  	${
      (!state.isLocked && questionState.guess !== null) ||
      (state.isLocked && questionState.answer === null)
        ? QuestionPublicResponses(questionState)
        : null
    }
  </div>`;
};
...
const QuestionLabel = questionState => {
  const text =
    questionState.answer !== null ? "The results are in" : "Waiting on results";
...
const QuestionPersonalChoices = (questionState, state, send) => {
  return yo`<div class="QuestionPersonalChoices">
      ...
      if (state.isLocked && questionState.answer !== null) {
        className +=
          " Button--" +
          (questionState.answer === index ? "" : "in") +
          "correct";
      }

      if (
        questionState.guess === index ||
        (state.isLocked &&
          questionState.guess === null &&
      ...
      const label = yo`<div>${choice
        .split(", ")
        .reduce((acc, str, index, strs) => {
      ...
      const image = questionState.images
        ? yo`<img src="${URL_ROOT}${cmimg(
      ...
      return yo`<div class="QuestionPersonalChoices-choice">
				<div class="${className}" onclick=${send.event("guess", {
...
const QuestionPersonalResult = questionState => {
  ...
  return yo`<div class="QuestionPersonalResult QuestionPersonalResult--${
    isCorrect ? "" : "in"
  }correct">
...
const QuestionPublicTitle = (questionState, state) => {
  ...
  if (state.isLocked && questionState.answer !== null) {
    text = `How did everybody ${
      questionState.guess === null ? "answer" : " else go"
    }?`;
  } else {
    text = `What does everybody${
      questionState.guess === null ? "" : " else"
    } think?`;
  }
...
const QuestionPublicResult = (questionState, state) => {
  if (state.isLocked && questionState.answer !== null) {
    if (questionState.guess === null) {
      return yo`<div class="QuestionPublicResult">
				${
          questionState.publicGuessPercentages[questionState.answer]
      ...
      return yo`<div class="QuestionPublicResult">
				${
          questionState.publicGuessPercentages[questionState.answer]
        }% of respondents got it right as well.
			</div>`;
    } else {
      return yo`<div class="QuestionPublicResult">
				<span class="is-negative">${
          questionState.publicGuessPercentages[questionState.guess]
        }%</span> of respondents also got this wrong.
				<br>
				<span class="is-positive">${
          questionState.publicGuessPercentages[questionState.answer]
        }%</span> got it right though.
			</div>`;
    }
  }
  ...
  return yo`<div class="QuestionPublicResult">
		${
      questionState.publicGuessPercentages[questionState.guess]
    }% of respondents agree with you${state.isLocked ? "" : " so far"}.
	</div>`;
};
